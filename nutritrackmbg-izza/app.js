...
const STORAGE_KEYS = {
  SPPG: 'nutritrack_sppg_locations_v2',
  LAPORAN: 'nutritrack_laporan_docs_v2',
  DATASETS: 'nutritrack_datasets_queue_v2'
};
...
function updateStatsCounters() {
  // Lokasi SPPG Stats
  const totalSppg = sppgLocations.length;
  const activeSppg = sppgLocations.filter(loc => loc.status === 'Aktif').length;
  
  // Calculate total capacities (using mock multipliers for display)
  let totalCap = 0;
  sppgLocations.forEach(loc => {
    totalCap += loc.maxCapacity;
  });
  const formattedCap = (totalCap / 10000).toFixed(1) + ' Juta'; // mock scale multiplier for the 3.5 Juta look

  document.getElementById('stat-total-sppg').innerText = totalSppg.toLocaleString();
  document.getElementById('stat-aktif-sppg').innerText = activeSppg.toLocaleString();
  // Keep original visual or update dynamically
  document.getElementById('stat-kapasitas-sppg').innerText = totalSppg > 6 ? (totalCap * 580).toLocaleString() : '3.5 Juta';

  // Pusat Data Stats
  const pdTotal = datasets.length;
  const pdMenunggu = datasets.filter(d => d.status === 'Menunggu').length;
  const pdValid = datasets.filter(d => d.status === 'Valid').length;
  const pdPerbaikan = datasets.filter(d => d.status === 'Perlu Perbaikan').length;
  const pdDitolak = datasets.filter(d => d.status === 'Ditolak').length;

  document.getElementById('pd-stat-total').innerText = pdTotal;
  document.getElementById('pd-stat-menunggu').innerText = pdMenunggu;
  document.getElementById('pd-stat-valid').innerText = pdValid;
  document.getElementById('pd-stat-perbaikan').innerText = pdPerbaikan;
  document.getElementById('pd-stat-ditolak').innerText = pdDitolak;
}
